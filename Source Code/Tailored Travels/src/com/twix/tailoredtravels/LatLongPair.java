 ...
 * @version 1.0
 * 
 *          This is the Coordinate class which defines the Coordinate data
 *          structure
 * 
 *          Each Coordinate possesses a : Label - Name of the Coordinate
 *          Longitude - horizontal dimension Latitude - vertical dimension
 */
public class LatLongPair {
	 ...
	 * @param x
	 */
	protected void setLong(double x) {
		this.x = x;
	}
	 ...
	 * @param y
	 */
	protected void setLat(double y) {
		this.y = y;
	}
	 ...
	 *            (longitude)
	 * 
	 *            This is the constructor for the Coordinate Object, takes in
	 *            the latitude and longitude (x & y respectively)
	 * 
	 */
	 ...
	 * the longitude and latitude to DEFAULT_VALUE Note: the default value for
	 * the label is 'null'
	 */
	public LatLongPair() {
	 ...
	 * 
	 *         This function calculates the distance between 2 coordinates in
	 *         miles dlon = lon2 - lon1 dlat = lat2 - lat1 a = (sin(dlat/2))^2 +
	 *         cos(lat1) * cos(lat2) * (sin(dlon/2))^2 c = 2 * atan2( sqrt(a),
	 *         sqrt(1-a) ) d = R * c (where R is the radius of the Earth)
	 */
	public double distance(LatLongPair other) {
		...
		double a = Math.pow(Math.sin(dlat / 2), 2)
				+ Math.cos(Math.toRadians(this.y))
				* Math.cos(Math.toRadians(other.y))
	...
	/**
	 * This is a function to TEST the methods and attributes of the Coordinate
	 * class
	 * 
	 * @param args
	 *            (none)
